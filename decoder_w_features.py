...
from prepared_datasets.vc_dataset import VCFeatureDataset, load_config, vc_feature_collate_fn


def move_feature_batch_to_device(batch, device):
    return {
...
def train_one_epoch(model, dataloader, optimizer, device, description="Training"):
    ...
    for batch in progress:
        ...
        coarse_mel, pred_mel = model(batch["content"], batch["speaker"], batch["prosody"], return_coarse=True)
...
def evaluate_loss(model, dataloader, device, description="Validation"):
    ...
    for batch in progress:
        batch = move_feature_batch_to_device(batch, device)
        pred_mel = model(batch["content"], batch["speaker"], batch["prosody"])
...
def save_checkpoint(
    ...
    loss,
    config,
    checkpoint_dir="model_checkpoints/decoder_w_features",
    val_loss=None,
):
    checkpoint_dir = Path(checkpoint_dir)
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = checkpoint_dir / f"decoder_w_features_epoch_{epoch:04d}.pt"
...
def build_training_objects(config=None, dataset=None, model=None, device=None):
    ...
    device = torch.device(device or get_device())
    dataset = dataset or VCFeatureDataset(config=config, split="train")
    model = (model or VCFeatureDecoder()).to(device)
...
def run_training(
    ...
    if training_config.get("smoke_test_training", False):
        smoke_dataset = VCFeatureDataset(config=config, split="train", limit=training_config["batch_size"])
    ...
    checkpoint_dir = training_config.get("model_checkpoint_dir") or "model_checkpoints/decoder_w_features"
...
class VCFeatureDecoder(nn.Module):
    def __init__(
        self,
        content_dim=768,
        speaker_dim=192,
        ...
        super().__init__()
        input_dim = content_dim + speaker_dim + prosody_dim
    ...
    def forward(self, content_latent, speaker_latent, prosody, return_coarse=False):
        ...
        if prosody.dim() == 4 and prosody.size(1) == 1:
            prosody = prosody.squeeze(1)
        if prosody.dim() != 3:
            raise ValueError(f"prosody must have shape [B, T, P], got {prosody.shape}")
        ...
        if prosody.size(0) != batch_size or prosody.size(1) != time_steps:
            raise ValueError(
                "content_latent and prosody batch/time dimensions must match: "
                f"content {content_latent.shape}, prosody {prosody.shape}"
            )

        speaker_latent = speaker_latent.unsqueeze(1).expand(-1, time_steps, -1)
        decoder_input = torch.cat([content_latent, speaker_latent, prosody], dim=-1)
...
if __name__ == "__main__":
    decoder = VCFeatureDecoder()
    content = torch.randn(2, 100, 768)
    speaker = torch.randn(2, 1, 192)
    prosody = torch.randn(2, 100, 3)
    coarse_mel, mel = decoder(content, speaker, prosody, return_coarse=True)
